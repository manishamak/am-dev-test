...
class APIDataDisplayBlock {
	...
	public function mm_display_API_data( $attr ) {
		...
		$html_structure = '';
		$custom_api_url = get_rest_url( null, CreateEndpoint::route_namespace . CreateEndpoint::endpoint );
		$api_data       = CreateEndpoint::get_accessible_API_data( $custom_api_url );
		$data_headers   = $api_data['data']->headers;
		$data_rows      = $api_data['data']->rows;

		if ( empty( $data_headers ) || empty( $data_rows ) ) {
			return '<p>' . esc_html( 'No data found in API', 'manisha-makhija' ) . '</p>';
		}
		...
		foreach ( $data_rows as $key => $record ) {
			...
			$html_structure .= $attr['date'] ? '<td>' . date( 'n/j/Y', $record->date ) . '</td>' : '';
			$html_structure .= '</tr>';
		}
	...
	public function mm_init_block_editor_assets() {
		...
		wp_localize_script(
			...
			array(
				'custom_endpoint' => CreateEndpoint::route_namespace . CreateEndpoint::endpoint,
			)
		);
	}
}
